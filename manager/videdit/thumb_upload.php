...
if( !in_array($ext, $allowed_ext) ) {
	header('Location: '.$url.'&fail=16');
	exit;
}

# 썸네일 폴더 (.THUMB) 없을시 만들기
if (!file_exists($startloc.$fdir.'/.THUMB')) {
    mkdir($startloc.$fdir.'/.THUMB', 0777, true);
...
// 파일 이동
if (!move_uploaded_file($_FILES['thumb_file']['tmp_name'], $startloc.$fdir.'/.THUMB/'.$fname_noext.'.jpg')) {
